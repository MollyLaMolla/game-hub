'use server'

import { revalidatePath } from 'next/cache'
import { getServerSession } from 'next-auth'
import { redirect } from 'next/navigation'
import profileIcons from '@/data/profileIcons'
import { authOptions } from '@/lib/auth'
import { prisma } from '@/lib/prisma'

export type AccountIdentityFormState = {
  formError?: string
  successMessage?: string
  fieldErrors?: {
    inGameName?: string
    tag?: string
  }
}

export type AccountAvatarFormState = {
  formError?: string
  successMessage?: string
  fieldErrors?: {
    avatarUrl?: string
  }
}

export type DeleteAccountFormState = {
  formError?: string
}

const allowedAvatarUrls = new Set(profileIcons.map(icon => icon.src))

function revalidateAccountSurfaces() {
  revalidatePath('/account')
  revalidatePath('/games')
  revalidatePath('/')
}

export async function updateAccountIdentity(
  _prevState: AccountIdentityFormState,
  formData: FormData
): Promise<AccountIdentityFormState> {
  const session = await getServerSession(authOptions)

  if (!session?.user?.id) {
    redirect('/login')
  }

  const inGameName = String(formData.get('inGameName') ?? '').trim()
  const tag = String(formData.get('tag') ?? '')
    .trim()
    .toUpperCase()

  const fieldErrors: AccountIdentityFormState['fieldErrors'] = {}

  if (!inGameName) {
    fieldErrors.inGameName = 'Enter an in-game name.'
  } else if (inGameName.length > 32) {
    fieldErrors.inGameName = 'The in-game name cannot exceed 32 characters.'
  }

  if (!tag) {
    fieldErrors.tag = 'Enter a tag.'
  } else if (tag.length > 5) {
    fieldErrors.tag = 'The tag cannot exceed 5 characters.'
  } else if (!/^[A-Z0-9]+$/.test(tag)) {
    fieldErrors.tag = 'Use only uppercase letters and numbers.'
  }

  if (Object.keys(fieldErrors).length > 0) {
    return { fieldErrors }
  }

  const duplicateProfile = await prisma.user.findFirst({
    where: {
      inGameName,
      tag,
      NOT: {
        id: session.user.id,
      },
    },
    select: {
      id: true,
    },
  })

  if (duplicateProfile) {
    return {
      formError: 'This in-game name and tag are already in use.',
    }
  }

  try {
    await prisma.user.update({
      where: {
        id: session.user.id,
      },
      data: {
        inGameName,
        tag,
        onboardingCompleted: true,
      },
    })
  } catch {
    return {
      formError: 'The profile could not be updated. Try again.',
    }
  }

  revalidateAccountSurfaces()

  return {
    successMessage: 'Name and tag updated successfully.',
  }
}

export async function updateAccountAvatar(
  _prevState: AccountAvatarFormState,
  formData: FormData
): Promise<AccountAvatarFormState> {
  const session = await getServerSession(authOptions)

  if (!session?.user?.id) {
    redirect('/login')
  }

  const avatarUrl = String(formData.get('avatarUrl') ?? '').trim()
  const fieldErrors: AccountAvatarFormState['fieldErrors'] = {}

  if (!allowedAvatarUrls.has(avatarUrl)) {
    fieldErrors.avatarUrl = 'Choose one of the available profile icons.'
    return { fieldErrors }
  }

  try {
    await prisma.user.update({
      where: {
        id: session.user.id,
      },
      data: {
        avatarUrl,
        onboardingCompleted: true,
      },
    })
  } catch {
    return {
      formError: 'The avatar could not be updated. Try again.',
    }
  }

  revalidateAccountSurfaces()

  return {
    successMessage: 'Avatar updated successfully.',
  }
}

export async function deleteAccount(
  _prevState: DeleteAccountFormState
): Promise<DeleteAccountFormState> {
  void _prevState

  const session = await getServerSession(authOptions)

  if (!session?.user?.id) {
    redirect('/login')
  }

  try {
    await prisma.user.delete({
      where: {
        id: session.user.id,
      },
    })
  } catch {
    return {
      formError: 'The account could not be deleted. Try again.',
    }
  }

  revalidateAccountSurfaces()
  redirect('/login')
}
